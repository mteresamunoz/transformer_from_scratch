...
│   ├── README.md                # index: which doc explains which file
│   ├── BIGRAM.md                # bigram deep dive (embeddings, loss, generate())
│   ├── ATTENTION.md             # self-attention + multi-head, with shape-by-shape walkthroughs
│   ├── BLOCK.md                 # feedforward + block (Add & Norm, residuals)
│   └── GPT.md                   # full architecture diagram + GPT-2 vs modern (RoPE/SwiGLU) choices
│
├── settings.py                 # per-model config: model class, architecture args, training hyperparams
├── train.py                    # training loop (--model bigram|gpt): forward, loss, backward, optimizer step
├── generate.py                 # generate text (--model bigram|gpt) from a trained checkpoint
└── README.md                   # this file
```
...
4. **`settings.py`** holds each model's own architecture arguments and training hyperparameters (the bigram and the GPT don't share a single hyperparameter, so each gets its own entry rather than forcing one shared shape).
5. **`train.py`** ties the data loader and the model together: it runs the training loop, evaluates the loss on train/validation splits, and saves checkpoints. See [`TRAINING.md`](TRAINING.md) for a walkthrough of what actually happens (and to whom) at each stage — initializing the model, training it, and later generating text from it.
6. **`generate.py`** loads a trained model and samples new text from it, one token at a time, using the same tokenizer/vocab defined in `data/`.

## Choosing a model: bigram vs GPT

Both `train.py` and `generate.py` accept a `--model` flag (`bigram` or `gpt`, default `bigram`), which selects an entry from `settings.py` — the right model class, its architecture args, and its own training hyperparameters, without touching any code:
...
Checkpoints are kept separate per model, under `checkpoints/<model>/` (`best.pt` plus one snapshot per evaluation) — training one model never overwrites the other's progress.
